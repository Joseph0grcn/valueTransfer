...
void CopyValues(std::vector<human>& v1, std::vector<human>& v2) {
    ...
    // Copy the first three elements from v2 to v1 using pointer arithmetic
    for (int i = 0; i < 3; ++i) {
        *(v1.data() + i) = *(v2.data() + i);
    }

    // Copy the next three elements from v1 to v2 using pointer arithmetic
    for (int i = 3; i < 6; ++i) {
        *(v2.data() + i) = *(v1.data() + i);
    }
}
